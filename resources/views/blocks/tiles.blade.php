...
<section data-gsap-anim="section" class="tiles -smt {{ $sectionClass }}">
	<div class="__wrapper c-main-wide bg-white py-20">
		<div class="c-main">
			...
			<div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
				<div class="flex flex-col justify-between order-2 lg:order-1">
					<div>
						...
						<h3 data-gsap-element="header" class="m-title">{{ strip_tags($tiles['title']) }}</h3>
						@endif
						@if (!empty($tiles['text']))
						<p data-gsap-element="txt" class="__txt text-xl pb-2">{{ strip_tags($tiles['text']) }}</p>
						@endif
					</div>

					@if (!empty($tiles['subtitle']))
					<h4 data-gsap-element="subheader" class="font-medium">{{ strip_tags($tiles['subtitle']) }}</h4>
					@endif
				</div>

				@if (!empty($tiles['image']))
				<div data-gsap-element="img" data-gsap-element="img" class="__img  order-1 lg:order-2">
			...
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mt-10">
				...
				<div data-gsap-element="card" class="__card b-border-light p-10">
